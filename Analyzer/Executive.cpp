...
bool sendFileToClient(string path,string FileName, int argc, char* argv[]) {
	try {
		...
		cout << "\n Html FilePath to be uploaded to client is : " << FileHtmPath << endl;
		FileSystem::File file(FileHtmPath);
		file.open(FileSystem::File::in, FileSystem::File::binary);
		if (!file.isGood())
		{   cout << "\n\n  can't open file " << FileHtmPath << endl;
			return false;}
		...
		file.close();
		cout << endl;
		Utilities::StringHelper::Title("Requirement 8 is met: Sent the file : " + FileHtmPath + " through stream of 2048 bytes each ");
		cout << "\n\n File : " <<  FileHtmPath << " content was sent to the client successfully" << endl;
	}
	catch (std::exception& exc)
	{	std::string exMsg = "\n  " + std::string(exc.what()) + "\n\n";
...
bool SendDependencyFilesToClient(string FileName,int argc,char* argv[]) {
	Utilities::StringHelper::Title("Demonstrating Requirement 10 is met :Lazy DownLoad Strategy");
	try {
		...
		for (it; it != CodeAnalysisExecutive::depmapforlazyloading.end(); it++) {
			...
			if (fileName_.compare(FileName) == 0) {
				...
				{
					cout << "\n File :" << FileName << "depends on " << str << endl;
...
void processGetMessageforGetCategories(HttpMessage &msg, int argc, char* argv[]) {
	...
	cout << "\nMessage to be sent is " << sndMsg.toString() << endl;
		...
		if (contentSize > 0) {
			...
			if (getMsgBody.find('$') != std::string::npos) {
				...
				if (FirstDollarSplit.find('$') != std::string::npos) {
					string CategoryName = FirstDollarSplit.substr(0, FirstDollarSplit.find('$'));
					string FileName = FirstDollarSplit.substr(FirstDollarSplit.find('$') + 1);
					cout << "\nServer needs to delete File = " << FileName << " And Category = " << CategoryName <<
						" message request from the client: \n";
...
bool ClientHandler::ProcessCommandLine(int argc, char * argv[])
{
	...
	cout << "\nInside ProcessCommandLine of ClientHandler" << endl;
	return true;
}
...
void displayCommandLineArgumentsHelp(int argc, char* argv[])
{
	...
	for (int i = 0; i < argc; i++) {
		...
		if (i == 7) {
			...
			cout << "        " << argv[i] << endl;
			/*if (!FileSystem::Directory::exists(argv[i])) {
				FileSystem::Directory::create(argv[i]);
			}*/
		}
		if (i == 8) {
...
{	
  //startHttpServer(argc, argv);
  //::SetConsoleTitle("Repository Server");
